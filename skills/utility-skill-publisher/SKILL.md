...
2. **Applying BEHAVIORAL ADDITIONs without user confirmation** — rules added "because they seem useful" measurably degrade task-specific performance.
...
[Mechanism: operator fills in — D1 query / Notion update / registry file]
Verify: confirm new entry is present before reporting complete.
```

| Skill state | Status |
|---|---|
| New skill (never INSTALLED) | STAGED — promote after one confirming session |
| Existing INSTALLED — additive update | INSTALLED — promote immediately |
| Existing INSTALLED — structural rewrite | STAGED — promote after one confirming session |

Additive: sections/examples added; no workflow steps removed. Structural: workflow changed, major sections replaced, trigger conditions changed.
