# Consumer Research Reference

Methodology for the Consumer Research mode of `utility-data-analyst`. Covers
consumer product and software purchase decisions: electronics, apparel,
kitchenware, software/apps, gardening, wellness, and everyday carry.
...
1. Prior Cycle Check
2. Phase 1 — Intake
3. Phase 2 — Source Routing
4. Phase 3 — Research Execution
5. Phase 3B — Retailer / SKU Drilldown
6. Phase 4 — Comparison Table
7. Phase 5 — Recommendation
8. Phase 6 — Drilldown / Refinement
9. Output Format
10. Quality Checks
11. Examples
...
Produce this block before any research begins. Prevents research spin on the
same product category within a session.

```
PRIOR CYCLE CHECK — [product category]
Prior research on this category this session: [yes — [output name] / no]
If yes — did the user act on that output (select a product or end the search)? [yes / no / unknown]
Reason for this new pass:
  (a) New constraint added — [what changed]
  (b) User wants a different category or price range — [what]
  (c) Prior recommendations did not satisfy — clarify what's missing before re-running
  (d) First pass on this category — confirm
```

If (c): ask what's missing before researching. Re-running the same pass with
no new constraint is the primary mechanism of consumer research spin.
...
- For software/apps: minimum functional requirements — what must it actually do,
  what platforms must it run on, what integrations matter. Ask before searching;
  wrong assumptions here waste the entire research pass.
...
may be wrong for light exercise. Never default to "low activity" — ask if not
stated.
...
Select primary sources based on category. Always consult at least 3 sources;
...
| Everyday Carry (EDC) | everydaycarry.com, Wirecutter, The Strategist |
...
| Spa / Wellness / Personal care | Wirecutter, Good Housekeeping, Strategist, Byrdie (skincare/beauty) |

**Source authority tiers (consumer product-specific):**

- **Tier 1 (independent testing, no affiliate pressure):** Consumer Reports,
  America's Test Kitchen, RTINGS, Outdoor Gear Lab — buys all tested products,
...
- **Tier 3 (supplemental, crowd-sourced or community):** Reddit category
  subreddits, verified buyer reviews, enthusiast forums

Tier 1 sources weight most heavily on reliability claims. Tier 2 on usability
and value. Tier 3 only for real-world longevity signals and edge cases.

**Critical tier weighting rule:** Consistent multi-owner failure reports in
Tier 3 are disqualifying even when Tier 1/2 sources are positive. Real-world
longevity data beats lab scores.
...
direct comparisons and any known failure modes or caveats not in list articles.
...
|---|---|---|
| 1 — Brand direct | Brand's own site | Fetch product page directly |
...
Amazon pricing, direct them to check the live page, or provide the ASIN link
and note the price must be verified manually.
...
that the price could not be verified for that retailer. Never substitute a
search snippet price. Try a different fetchable retailer or flag as unverified.
...
**5e — Mandatory per-unit math block.** Before delivering any price for a
consumable product (supplements, coffee, food), compute:
...
- Color/size selectors are populated and preferred options are available
...
it directly on the first pass. Never keep searching across multiple turns.
Redirect immediately to the next-best option or a competing brand.
...
- Price = current street price, not MSRP. Flag if price varies significantly.
  Use `[unverified — check live]` if not fetched this session.
...
**Top Pick: [Product Name]** `[VERIFIED / PARTIAL]`
1–2 sentences on why this wins for the user's stated needs. Must cite specific
evidence (source + finding), not generic praise. Confidence label required:
- `[VERIFIED]`: N≥2 Tier 1/2 sources converge on this product for the user's use case
- `[PARTIAL]`: Single source, or sources converge on quality but not specifically for this use case
...
- **Constraint refinement:** User adds a constraint (activity level, inseam,
  feature requirement) → re-run Phases 3–5 with updated parameters.
...
2. Comparison table
3. Recommendation block (with confidence labels)
4. Drilldown offer

No preamble about methodology. Lead with the table.
...
- Per-unit math shown explicitly for any consumable product price
...
- Top Pick carries a confidence label ([VERIFIED] or [PARTIAL]) with citation
...
head-to-head. Table of 4 knives. Top Pick: `[VERIFIED]` — ATK and Wirecutter
both name it for home cooks in this price range. Runner-up: Wirecutter pick
for smaller hands.
...
flag IPX rating as key spec. Top Pick: `[VERIFIED]` if both sources agree on
sport use; `[PARTIAL]` if only RTINGS.
...
surface 3 products. Confidence label on Top Pick based on available Tier coverage.
...
and surface 1–2 competitors at the same price to validate or reframe.
...
remove options not rated for that activity level. Rerank everything.

**Price verification:** Finalist is Sports Research D3+K2, 60ct, $23.95
→ Fetch sportsresearch.com product page directly. Price confirmed $23.95. Math:
60 caps ÷ 1/day = 60 days = 2 months; $23.95 ÷ 2 = $11.98/month.
Never say "~$8/month" — always compute and show work.

**Amazon pricing:** User asks for Amazon price on any product.
→ "Amazon prices can't be verified by fetch — Amazon blocks AI crawlers.
Check live: amazon.com/dp/[ASIN]"

**Prior cycle check fires:** User asked for headphone recommendations 10 minutes
ago and received a table. Now asks "actually can you find me better options?"
→ PRIOR CYCLE CHECK: ask what's missing from the prior output before re-running.
Did budget change? Use case shift? Prior picks out of stock? Clarify before
searching again.
